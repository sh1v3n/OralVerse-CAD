...
Dataset layout expected (the challenge zip extracts like this):
    <data_dir>/
        <case_id>/
...
Both layouts are handled automatically: the script pairs every .obj
with the .json that shares the same stem in the same directory.
...
def compute_features(mesh: trimesh.Trimesh) -> np.ndarray:
    """Return (F, 9) float32 feature matrix for a loaded trimesh.

    Features (per face):
      0-2  centroid (X, Y, Z) — normalised to [-1, 1]
      3-5  unit face normal (Nx, Ny, Nz)
      6    log face area (normalised)
      7-8  estimated principal curvatures κ₁, κ₂
    """
    F = len(mesh.faces)
    ...
    # — principal curvatures (F, 2) —
    # Approximate via per-vertex discrete curvature averaged to faces.
...
def _face_curvatures(mesh: trimesh.Trimesh) -> np.ndarray:
    """Estimate two principal curvatures per face — fast approximation.

    Uses the discrete mean curvature H and Gaussian curvature K at each
    vertex (from the angle-deficit method), then averages the three vertex
    values to the face centroid.
    ...
    # Vertex mixed areas and angle sums (for discrete curvature)
    ...
    face_areas = np.linalg.norm(cross, axis=1) / 2.0  # (F,)

    # Angles at each vertex of each face
    def _angle(a, b):
        cos_t = np.clip(
    ...
    # Mean curvature H per vertex — use cotangent Laplacian approximation
    # Simple proxy: use the magnitude of the Laplacian of position
    laplacian = np.zeros((V, 3), dtype=np.float64)
    weights = np.zeros(V, dtype=np.float64)
    for fi in range(F):
        for local_i, (a, b, c) in enumerate([(0,1,2),(1,2,0),(2,0,1)]):
            vi = faces[fi, a]
            vj = faces[fi, b]
            vk = faces[fi, c]
    ...
    H_vert = np.linalg.norm(laplacian, axis=1) / 2.0  # unsigned
    ...
    # Clip + normalise to a stable range
    k1 = np.clip(k1, -50.0, 50.0)
    k2 = np.clip(k2, -50.0, 50.0)
    k1 = (k1 / 50.0).astype(np.float32)
    k2 = (k2 / 50.0).astype(np.float32)

    return np.stack([k1, k2], axis=1)  # (F, 2)
...
def load_labels(json_path: Path) -> np.ndarray:
    ...
    The file may be:
      {"labels": [0, 11, 11, ...]}       ← array at key "labels"
      [0, 11, 11, ...]                    ← bare array
      {"id": "...", "labels": [...]}      ← with extra metadata
    """
...
def fdi_to_class(fdi_labels: np.ndarray, arch: str) -> np.ndarray:
    ...
    # Any FDI not in the mapping (e.g. opposite arch tooth in a mixed scan)
    # stays 0 (treated as gingiva — conservative fallback)
    return cls
...
def _detect_arch(path: Path) -> str | None:
    ...
    if "upper" in name or "maxillar" in name or "upper" in path.parts:
        return "upper"
    if "lower" in name or "mandibular" in name or "lower" in path.parts:
        return "lower"
    return None
...
def process_scan(
    ...
    try:
        ...
        features = compute_features(mesh)          # (F, 9)
        cls_labels = fdi_to_class(fdi_labels, arch)  # (F,)
...
def write_splits(
    processed: list[tuple[str, str]],  # [(arch, stem), ...]
    out_dir: Path,
    ...
    print(f"\nSplits written: {len(splits['train'])} train / {len(splits['val'])} val / {len(splits['test'])} test")


# ── CLI ───────────────────────────────────────────────────────────────────────

def main() -> None:
    parser = argparse.ArgumentParser(description="Preprocess 3DTeethSeg22 for MeshSegNet")
    parser.add_argument("--data_dir", required=True, type=Path, help="Root of downloaded dataset")
    parser.add_argument("--out_dir",  default=Path(__file__).parent / "data", type=Path)
    parser.add_argument("--workers",  default=1, type=int, help="Parallel workers (requires joblib)")
    ...
    print(f"Found {len(pairs)} scans under {args.data_dir}")
    ...
    for obj_path, json_path, arch in tqdm(pairs, desc="Processing"):
        success = process_scan(obj_path, json_path, arch, args.out_dir)
        if success:
            ok += 1
    ...
    if processed:
        write_splits(processed, args.out_dir)

    # Print class distribution summary
    _print_class_stats(args.out_dir, processed[:20])


def _print_class_stats(out_dir: Path, sample: list[tuple[str, str]]) -> None:
    """Print face-count per class across a small sample to spot imbalance."""
    if not sample:
        return
    counts: dict[int, int] = {}
