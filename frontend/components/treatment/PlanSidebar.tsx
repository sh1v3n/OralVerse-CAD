    ...
    <aside className="space-y-4">
        ...
        <div className="space-y-2">
          {plan.analysis.issues.map((issue, index) => (
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl border border-white/5 bg-white/[0.025] px-3 py-2.5 text-left hover:bg-white/[0.05]"
            >
              <span>
                <span className="block text-sm">{ISSUE_LABELS[issue.type] ?? issue.type}</span>
                <span className="text-[11px] capitalize text-slate-500">{issue.arch} arch</span>
              </span>
      ...
      <section className="ortho-panel">
        <div className="mb-3 flex items-center justify-between">
          ...
          {selected && (
            <button onClick={() => selectTooth(null)} className="text-xs text-slate-500 hover:text-white">
              Clear
            </button>
          )}
        </div>
  ...
  return (
    <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
      {movements.slice(0, 10).map((movement) => (
        <div key={movement.fdi} className="flex items-center gap-3 rounded-xl bg-white/[0.025] p-2.5">
          <span className="grid h-8 w-8 place-items-center rounded-lg bg-cyan-300/10 text-xs font-semibold text-cyan-300">
            {movement.fdi}
          </span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-xs capitalize text-slate-200">{movement.direction}</p>
...
function ToothDetails({
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm capitalize text-slate-200">{movement.tooth_name}</p>
      ...
      {movement.attachment && (
        <div className="rounded-xl border border-cyan-300/15 bg-cyan-300/[0.04] p-3">
          <p className="text-[10px] uppercase tracking-wider text-cyan-300">Attachment</p>
          <p className="mt-1 text-xs capitalize">{movement.attachment.type}</p>
          <p className="mt-1 text-[11px] text-slate-500">{movement.attachment.reason}</p>
        </div>
      )}
    </div>
  );
